...
ORDER_LIST_URL_TEMPLATE = (
    "https://{admin_id}.cafe24.com/admin/php/shop1/s_new/order_list.php"
    "?rows=20&searchSorting=order_desc&btnDate=9999&date_type=order_date"
    "&memberType=1&shop_no_order=1&orderStatusPayment=all"
    "&orderStatusNotPayCancel=N&orderStatusCancel=N&orderSearchCancelStatus=all"
    "&orderStatusExchange=N&orderSearchExchangeStatus=all"
    "&orderStatusReturn=N&orderStatusRefund=N&orderSearchRefundStatus=all"
    "&orderSearchShipStatus=all"
    "&orderStatus[]=all&orderStatus[]=N10&orderStatus[]=N20"
    "&orderStatus[]=N22&orderStatus[]=N21&orderStatus[]=N30&orderStatus[]=N40"
    "&incoming=T&realclick=T&initSearchFlag=T"
)
...
def click_first_download_button(page1, save_dir: str) -> str:
    ...
    if target_table.count() == 0:
        raise RuntimeError("다운로드 리스트에서 '쇼핑몰' 컬럼헤더가 있는 테이블을 찾지 못했습니다.")

    first_row = target_table.locator("tbody tr").first
    if first_row.count() == 0:
        raise RuntimeError("다운로드 리스트 테이블에서 첫 번째 데이터 행을 찾지 못했습니다.")
...
def download_cafe24_excel(
    ...
    with sync_playwright() as p:
        ...
        target_url = (
            ORDER_LIST_URL_TEMPLATE.format(admin_id=cred.admin_id)
